...
self.addEventListener('install', event => {
  ...
  event.waitUntil(caches.open(CACHE_NAME).then(cache => {
    ...
    return cache.addAll(['/', OFFLINE_URL, '/icons/icon-192x192.png', '/icons/icon-96x96.png']).catch(err => {
...
self.addEventListener('notificationclick', function (event) {
  ...
  const urlToOpen = ((_event$notification$d = event.notification.data) === null || _event$notification$d === void 0 ? void 0 : _event$notification$d.url) || ((_event$notification$d2 = event.notification.data) === null || _event$notification$d2 === void 0 ? void 0 : _event$notification$d2.clickAction) || '/';
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(caches.match(event.request).then(cachedResponse => {
    ...
    return fetch(event.request).then(response => {
      ...
      const shouldCache = url.includes('/media/') || url.includes('/icons/') || url.includes('/images/');
      if (shouldCache) {
...
self.addEventListener('pushsubscriptionchange', event => {
  ...
  event.waitUntil(self.registration.pushManager.subscribe(event.oldSubscription.options).then(subscription => {
